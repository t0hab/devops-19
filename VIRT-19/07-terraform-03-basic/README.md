...
Terraform will perform the following actions:
  ...
  + resource "yandex_compute_instance" "vm_count" {
      ...
      + metadata                  = {
          + "ssh-keys" = <<-EOT
                ubuntu:ssh-rsa AAAA.........= t0hab@t0hab-pc
            EOT
        }
      + name                      = "prod-0"
...
Note: You didn't use the -out option to save this plan, so Terraform can't guarantee to take exactly these actions if you run "terraform apply" now.
 ~/GIT/my_rep_netology/devops-19/VIRT-19/07-terraform-03-basic | main +24 !6 ?4                                                                                                            prod tf | 14:49:07 
> 
```
```bash
...
external_ip_address_vm_for_each = toset([
  "130.193.49.4",
  "158.160.35.238",
])
internal_ip_address_vm_count = [
...
internal_ip_address_vm_for_each = toset([
  "192.168.10.16",
  "192.168.10.5",
])
```
